...
- **Short pulse** (~6 μs): Represents bit `1`
- **Long pulse** (~60 μs): Represents bit `0`
...
3. Duration < 20 μs = bit `1`, duration > 20 μs = bit `0`
...
1. **Identify Rising Edges**: Find SCK transitions from 0→1
...
## 🔗 Protocol 3: I2C (Inter-Integrated Circuit)
...
```
┌─────────────┐
│  DQ Signal  │ → "read and decrypt the SPI bus data using the XOR key: icy"
└─────────────┘
       │
       ↓
┌─────────────┐
│ SPI Signal  │ → "read and decrypt the I2C bus data using the XOR key: bananza.
└─────────────┘    the temperature sensor address is 0x3C"
       │
       ↓
┌─────────────┐
│ I2C Signal  │ → Temperature: 32.84°C, Pressure: 1013 hPa
└─────────────┘    Humidity: 45%/85%, Light: 3450/3850 lux
```
...
### Keys Used
- **DQ → SPI**: Key = `"icy"` (3 bytes)
- **SPI → I2C**: Key = `"bananza"` (7 bytes)
...
def decode_dq_signal(filename):
    ...
    - Bits are read from HIGH→LOW transitions
    - Short low pulse (~6 μs) = bit 1
    - Long low pulse (~60 μs) = bit 0
...
def decode_spi(sck_signals, mosi_signals):
    ...
    # Find rising edges of clock (0→1 transitions)
    clock_rising_edges = []
...
def main():
    ...
    for idx, trans in enumerate(transactions):
        ...
        print(f"  Time: {trans['start']} → {trans['stop']}")
        ...
        if address == 0x3C and len(trans_bytes) > 1:
            ...
            print(f"  ✓ From sensor 0x3C - collected {len(trans_bytes)-1} data bytes")
...
def find_i2c_transactions(scl_signals, sda_signals):
    ...
    # Find START conditions (SDA 1→0 while SCL=1)
    for i in range(len(sda_signals) - 1):
        if sda_signals[i]['v'] == 1 and sda_signals[i+1]['v'] == 0:
            start_time = sda_signals[i+1]['t']
            ...
            if scl_high:
                # Find corresponding STOP (SDA 0→1 while SCL=1)
                for j in range(i+1, len(sda_signals) - 1):
                    if sda_signals[j]['v'] == 0 and sda_signals[j+1]['v'] == 1:
                        stop_time = sda_signals[j+1]['t']
...
async def main():
    ...
    try:
        ...
        print(f"\n✓ Message saved to i2c_decrypted_message.txt")
